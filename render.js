...
Hyumu.Render = (function () {
  ...
  function renderEmployeeScreen(container, doc, handlers) {
    const rows = doc.employees.map((emp) => `
      <div class="employee-card" data-id="${emp.id}">
        ...
        <div class="specific-off">
          ...
          <button type="button" class="btn-add-date" data-id="${emp.id}">개인 휴무 추가</button>
    ...
    container.querySelectorAll('.btn-add-date').forEach((btn) =>
      btn.addEventListener('click', () => {
        const dateInput = container.querySelector(`.emp-date-input[data-id="${btn.dataset.id}"]`);
        if (dateInput.value) {
          handlers.onAddSpecificOff(btn.dataset.id, dateInput.value);
          dateInput.value = '';
        }
      })
    );
